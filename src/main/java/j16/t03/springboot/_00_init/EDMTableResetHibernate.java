package j16.t03.springboot._00_init;
//package j16.t03.springboot.init;
//
///*  
//    程式說明：建立表格與設定初始測試資料。
...
//					customerBean.setCustId(Integer.parseInt(token[12]));
//					ob.setCustomerBean(customerBean);
//
//					session.merge(ob);
//					count++;
...
//					RentProductBean rentProductBean = new RentProductBean();
////					rentProductBean.builder().prodId(Integer.parseInt(token[12])).serialNumber(token[13]);
//					rentProductBean.setProdId(Integer.parseInt(token[12]));
//					rentProductBean.setSerialNumber(token[13]);
//					rib.setRentProductBean(rentProductBean);
